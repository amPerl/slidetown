...
impl Obj {
    pub fn write_to_files(&self, obj_path: PathBuf, mtl_path: PathBuf) -> anyhow::Result<()> {
        ...
        if !self.materials.is_empty() {
            ...
            for (name, material) in self.materials.iter() {
                writeln!(&mut out_mtl, "newmtl {}", name)?;
    ...
    pub fn visit_tri_shape(
        ...
        for &property_ref in tri_shape.property_refs.iter() {
            ...
            match property {
                Block::NiMaterialProperty(material_property) => {
                    material = Some((
                        format!("{}_NiMaterialProperty{}", label, property_ref.0),
                        ObjMaterial {
        ...
        if let Some((material_name, mut material)) = material {
            ...
            let existing_material = self
                .materials
                .iter()
                .find(|(_, existing_material)| existing_material == &&material);

            if let Some((existing_name, _)) = existing_material {
                mesh.material_name = Some(existing_name.clone());
            } else {
                mesh.material_name = Some(material_name.clone());
                self.materials.insert(material_name.clone(), material);
            }
        }
    ...
    pub fn visit_texturing_property(
        ...
        if texturing_property.has_base_texture && texturing_property.base_texture.is_some() {
        ...
        } else if texturing_property.num_shader_textures > 0 {
            // TODO: handle blending multiple textures. For now, just use the first one
            let shader_textures = &texturing_property.shader_textures;
            let shader_texture = shader_textures.first().unwrap();

            if let Some(tex_desc) = shader_texture.map.as_ref() {
                let source = tex_desc
                    .source_ref
                    .get(&nif.blocks)
                    .expect("invalid property ref");
